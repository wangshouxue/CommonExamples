...
/**
 * @Description: 局部复制
 * 参考：https://github.com/ITxiaoguang/SelectTextHelper
 */
class SelectTextHelper (builder: Builder) {
    ...
    private var mRootTouchListener: View.OnTouchListener? = null
    ...
    companion object {
         ...
         */
        @Volatile
        var emojiMap: MutableMap<String, Int> = java.util.HashMap()
...
//                        if (mSpan is SelectImageSpan) {
                        if (mSpan is CenterImageSpan) {//项目里是CenterImageSpan
                            return true
                        }
                    }
                }
    ...
    interface OnSelectListener {
        ...
        fun onLongClick(v: View?,mSelectTextHelper:SelectTextHelper) // 长按textView
...
//        val spanStr =SmileUtils.getSmiledText(mContext,
//            newContent,
//            mTextView,
//            R.color.color_b0edff
//        )
...
//        mTextView.setText(spanStr, TextView.BufferType.SPANNABLE)
        ...
        mTextView.setOnClickListener {
            ...
            if (null != mSelectListener && (null == mOperateWindow || !(mOperateWindow?.isShowing?:false))) {
        ...
        mTextView.setOnLongClickListener {
            ...
            // 根布局监听
            mRootTouchListener = View.OnTouchListener { _, _ ->
                reset()
                mTextView.rootView.setOnTouchListener(null)
            ...
            if (null != mSelectListener) {
                mSelectListener?.onLongClick(mTextView,this)
            }
            true
    ...
    private fun showCursorHandle(cursorHandle: CursorHandle?) {
        ...
        val offset = if (cursorHandle?.isLeft?:false) mSelectionInfo.mStart else mSelectionInfo.mEnd
        ...
        // 把右游标底部线坐标定位在上一行
        if (!(cursorHandle?.isLeft?:false) && mSelectionInfo.mEnd != 0 && x == 0) {
    ...
    private fun setEmojiBackground() {
        ...
        setEmojiBackground(
            mSpannable?.subSequence(mSelectionInfo.mEnd, mSpannable?.length?:0) as Spannable,
            Color.TRANSPARENT
        )
    }
...
//            var posY = startY + mTempCoors[1] - mHeight
            //距离近些
            var posY = startY + mTempCoors[1] - mHeight/3*2
            if (posY < 0) posY = 0
            ...
            if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_DOWN) {
                ...
                if (links.isNotEmpty()) {
                    if (action == MotionEvent.ACTION_UP) { // 长按
                        if (System.currentTimeMillis() - downLinkTime < ViewConfiguration.getLongPressTimeout()) {
